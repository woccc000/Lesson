...
class Warehouse:
    ...
    def truck_arrived(self, truck):
        ...
        print('{}: Прибыл новый грузовик {}'.format(self.name, truck))
...
class Vehicle:
    ...
    def __str__(self):
        return '{} топлива - {}'.format(self.model, self.foul)
...
class AutoLoader(Vehicle):
    ...
    def act(self):
        ...
        elif self.load == 'loader':
            self.load()
        else:
            self.unload()
        
    def load(self):
        truck_cargo_res = self.truck.body_space - self.truck.cargo
        if truck_cargo_res < self.bucket_capacity:
            self.warehouse.content -= self.bucket_capacity
            self.truck.cargo += self.bucket_capacity
        else:
            self.warehouse.content -= truck_cargo_res
            self.truck.cargo += truck_cargo_res
